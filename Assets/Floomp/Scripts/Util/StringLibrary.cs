public static class StringLibrary
{
    //Input
    public static readonly string PanHorizontal = "PanHorizontal";
    public static readonly string PanVertical = "PanVertical";
    public static readonly string Zoom = "Zoom";
    public static readonly string LeftClick = "LeftClick";
    public static readonly string MouseX = "Mouse X";
    public static readonly string MouseY = "Mouse Y";
    public static readonly string MouseScrollWheel = "Mouse ScrollWheel";

    // Units
    public static readonly string ScorableUnitTag = "ScorableUnit";

    // Buildings
    public static readonly string BuildableAreaTag = "BuildableArea";

    // Pools
    public static readonly string PoolSuffix = " Pool";

    // UI
    public static readonly string HealthBar = "HealthBar";

    // Animations
    public static readonly string animationRef = "animation,";
    public static readonly string idleAnimation = "Idle";
    public static readonly string moveAnimation = "Move";
    public static readonly string attackAnimation = "Attack";
    public static readonly string damageAnimation = "Damage";
    public static readonly string deathAnimation = "Death";

}
